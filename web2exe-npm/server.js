...
// Installs a custom server.js's declared dependencies. --ignore-scripts is
// deliberate: this runs arbitrary uploaded package.json contents on our own
// server, and skipping install/postinstall scripts closes off the obvious
// route to running arbitrary code that way. The tradeoff is that packages
// needing a native build step (node-gyp etc.) won't work — pure-JS packages
// like express, ws, or socket.io are unaffected.
    ...
    execFile(
      ...
      (err, stdout, stderr) => {
        if (err) {
          ...
          return reject(new Error('Installing your server.js dependencies failed:\n' + tail));
        }
        resolve();
      }
    );
...
app.post('/build', uploadFields, async (req, res) => {
  ...
  try {
    ...
    // If there's a custom server.js, a package.json sitting right next to
    // it (same folder) is where its npm dependencies would be declared —
    // set it aside too, instead of letting it fall into public/ like any
    // other file.
    let customServerPkgJson = null;
    if (customServer) {
      const candidate = path.join(path.dirname(customServer), 'package.json');
      if (allFiles.includes(candidate)) customServerPkgJson = candidate;
    }

    // Move everything except the chosen server.js (and its package.json,
    // if any) into public/.
    fs.mkdirSync(publicDir, { recursive: true });
    for (const full of allFiles) {
      if (full === customServer || full === customServerPkgJson) continue;
    ...
    if (customServer) {
      ...
      if (customServerPkgJson) {
        let parsed;
        try {
          parsed = JSON.parse(fs.readFileSync(customServerPkgJson, 'utf8'));
        } catch (e) {
          cleanup(workDir);
          return res.status(400).json({ error: 'Could not parse the package.json next to your server.js: ' + e.message });
        }
        if (parsed.dependencies && typeof parsed.dependencies === 'object' && Object.keys(parsed.dependencies).length) {
    ...
    if (userDependencies) generatedPackageJson.dependencies = userDependencies;
    fs.writeFileSync(path.join(workDir, 'package.json'), JSON.stringify(generatedPackageJson, null, 2));

    if (userDependencies) {
      // Your server.js gets require()'d directly into the Electron main
      // process — it isn't run through npm itself, so any package beyond
      // core Node modules has to actually be installed here first, from
      // just the dependencies you declared.
    ...
    const appPaths = await packager({
      ...
      out: outDir,
      overwrite: true,
      // Skip asar packing whenever there's a custom server: some npm
      // packages ship native (.node) binaries that can't be loaded from
      // inside an asar archive, and there's no reliable way to know in
      // advance whether a given dependency tree has one. Shipping the
      // folder unpacked sidesteps the whole problem.
      asar: !customServer,
      prune: true,
      electronVersion
    });
