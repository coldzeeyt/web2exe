...
main process, so it should listen on `process.env.PORT`. Core Node
modules always work. If it needs npm packages too (Express, etc.), drop
a `package.json` in the same folder listing them under `dependencies` —
the build installs them for real before packaging. Native/compiled
packages (ones needing `node-gyp` or an install script) aren't
supported, since installs run with `--ignore-scripts` for safety —
that's fine for the vast majority of packages (Express, ws, socket.io,
and friends are all pure JS).
